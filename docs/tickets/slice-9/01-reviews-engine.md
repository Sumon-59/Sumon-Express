...
- [ ] Eligibility matrix: delivered 201; pending/shipped 403; cancelled 403; never-bought 403; anonymous 401
- [ ] Duplicate review 400; unique index present as the race backstop
- [ ] Rating validation: 0 / 6 / 3.5 / missing named 400s; comment cap enforced
- [ ] Denormalization: create, second reviewer (5+2 → 3.5), edit, delete (to zero) — all proven via the PUBLIC product endpoint
- [ ] Owner-only edit/delete (other user 403, unknown 404)
- [ ] Listing: public, newest first, wrapper, name present, email absent
- [ ] Eligibility read: all four states
