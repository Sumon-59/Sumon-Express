...
| 7 — Variants & galleries | ✅ 2026-09-11 — one optional option axis (name/stock/price-override values), top-level stock as the atomically-maintained SUM (dual-$inc, $elemMatch guard), axis rules folded INTO validateProductData after a hard review violation, variant-aware cart identity + picker + gallery + axis editor; review caught the plain-cancel-after-axis mirror divergence and the bare-[] no-op; 164 tests (130 BE + 34 FE) |
...
| 9 — Reviews & ratings | ✅ 2026-09-11 — verified-purchase eligibility (DELIVERED order = the rule, named 403), one review per user per product (compound unique index IS the rule, E11000→400), ratingAvg/ratingCount recomputed never incremented (store what you claim, recompute what you derive), stars + reviews section + eligibility-driven form; deploy probe caught aggregate()'s no-schema-defaults gap on legacy docs ($ifNull fix, raw-doc pinned); 188 tests (154 BE + 34 FE) |
| 10 — Store settings & theming | ✅ 2026-09-11 — StoreSettings SINGLETON at a fixed _id (review caught the false "{} upsert is race-safe" claim — unique-indexed filter or nothing; concurrent test pins it), public read that IS a read (review: GET was writing updatedAt per visitor), validateSettingsData choke point, one-CSS-variable theming (--primary override recolors the whole shadcn surface), admin form + draft-driven live preview; prod probe: full owner round-trip incl. named-400 refusal; 200 tests (164 BE + 36 FE) |
| 11 — Online payments (SSLCommerz) | ✅ 2026-09-11 — PaymentProvider trust boundary (provider attests, controller judges), verified-IPN-only paid path with GUARDED atomic transitions; review caught the cross-transaction replay (validator tran_id never checked against the order's attempt — the fake was hiding it) + cancelled-order IPNs + cod/online whitelist; deploy probes caught CLIENT_URL never set on Render (localhost in gateway redirects); PARKED: sandbox-credential gateway round-trip (Sumon registers) + redirect-host fix awaiting stalled Render deploy; 213 tests (176 BE + 37 FE) |
| 12 — Shipping & timeline | ✅ 2026-09-11 — shippingMethods on the settings singleton (choke-point validated, slug keys, whole-taka fees), orders snapshot {key,label,fee,eta} and price (subtotal − discount) + fee server-side (goods-only discounts pinned), history [{status,at}] written ONLY via recordStatus (one door, both cancels, illegal moves append nothing), checkout radio + OrderTimeline + admin fee editor; review caught the drawer's missing Delivery row, deploy-window settings skew, Bangla-label slug failure; probes verified live (named-400 seam + fee round-trip) and traced the localhost-redirect mystery to CLIENT_URL=localhost in Render (code now refuses it); 224 tests (185 BE + 39 FE) |
| 13 — Transactional email | ✅ 2026-09-11 — mailer boundary (fake/console/Resend-over-fetch, zero new deps), fire-and-forget enforced in ONE throw-proof dispatch (mail can never fail or delay a request — pinned via failNext), snapshot-only email bodies, triggers: confirmation + status moves + both cancel doors; review caught the unfalsifiable fee assertion, a mailer⇄resend import cycle, and made the no-payment-email silence a spec-owned decision; probe: mail-status live answering console (flips to resend when a key lands); 192 BE tests |
| 14 — Auth & authz hardening | ✅ 2026-09-11 — password reset (SHA-256-at-rest single-use expiring token, constant 200, global session revocation), password change (current proved, one shared min-8 policy — register had NONE), login notification emails, staff RBAC (requireRole generalizing requireAdmin, permission map in ONE router file, closed-set role writes); threat-model review added per-route rate buckets, a redacting production console mailer (reset links must never reach platform logs), and caught analytics disagreeing with the census about staff; probes: forgot-password live, requireRole regression clean. Scoped out with reasons: email verification, device lists. 243 tests (204 BE + 39 FE) |
| 15 — Stretch goals (low-stock alerts) | ✅ 2026-09-11 — picked by appetite from the stretch list (multi-currency, wishlist, low-stock alerts, second payment provider — chose the one needing no new external account); global lowStockThreshold on the settings singleton (0 disables), edge-triggered detection reusing the Slice 7 stock engine's claimItemStock (now returns the post-claim stock of the unit that moved), one batched email per admin per order via the Slice 13 dispatch door, mailed ONLY after order.save() commits (never on a claim later rolled back); a live low-stock survey endpoint (route-ordering gotcha: registered before /products/:id) + admin dashboard panel; review caught a vacuous rollback test (its lever refused before the claim loop ever ran — replaced with a genuine claimed-then-restored case) and a divergence-prone duplicated axis check in the survey query; prod probe: the public /api/settings response now carries lowStockThreshold live. 219 backend tests (204 + 15), 39 frontend tests |
